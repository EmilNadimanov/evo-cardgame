...
import evo.cardgame.cardGames.Player
import fs2.Stream
...
class Session[F[+ _]: Concurrent, CardType <: Card](
  ...
  // repeating rounds of a card game
  def run() = {
    val parsingPlayerActions = Stream.eval {
      for {
        currentGameState <- gameStateRef.get
        _ <- parsePlayerActions(currentGameState.players.values)
      } yield ()
    }.repeat
    ...
    Stream(dealAndMakeMoves, parsingPlayerActions).parJoinUnbounded.compile.drain
  }

  private def parsePlayerActions(players: Iterable[Player[F, CardType]]): F[Unit] = {
    def reactToMovesPipe(playerActions: Stream[F, PlayerMessage]): Stream[F, Unit] = {
      playerActions
        .evalMap {
          case PlayerMessage.Disconnect(userName) =>
            Announce(s"$userName left the game. The game will end soon").pure[F]
          ...
          case PlayerMessage.MakeMove(userName, move) =>
            movesRef.modify { map =>
              map.get(userName) match {
                ...
                case None =>
                  (map.updated(userName, move), Announce(s"$userName decided to $move"))
              }
            }
        }
    ...
    players
      ...
      .foldLeft[Stream[F, PlayerMessage]](Stream.empty) {
        case (acc, stream) => Stream(acc, stream).parJoinUnbounded[F, PlayerMessage]
      }
  ...
  private def dealIfNeeded(currentGameState: SimpleCardGame[F, CardType]): F[SimpleCardGame[F, CardType]] =
    for {
      ...
      _ <- if (needToDeal) {
        serverTopic
          .publish {
            Stream.emits(
              maybeUpdatedGameState.players.values.toList
                .map { p =>
                  SimpleText(p.name, s"Dear ${p.name}, your cards are: ${p.hand.cards.mkString(", ")}")
                }
            )
          }
          .compile
  ...
  private def makeMoves(gameState: SimpleCardGame[F, CardType]): F[SimpleCardGame[F, CardType]] =
    for {
      movesToMake <- movesRef.modify { existingMoves =>
        val allPlayers         = gameState.players.values.map(_.name)
        val allPlayersMadeMove = allPlayers.forall(existingMoves.keySet.contains)
              ...
              .publish {
                Stream.emits(
                  state.players.values.toList
                    .map { p =>
                      Announce(s"${p.name} has ${p.score} poins")
                    }
